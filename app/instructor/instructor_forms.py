...
class UpdateCourseForm(FlaskForm):
    ...
    section =  StringField('Section', validators=[Length(min=1, max=5)])
    year_taken = StringField('Year the course is being taught', validators=[Length(min=1, max=4), Regexp('^\d{4}$', message='Please enter a valid 4-digit year')])
    term_taken = SelectField('Term the course is being taught',
                             choices=[('A', 'A-Term'), ('B', 'B-Term'), ('C', 'C-Term'), ('D', 'D-Term'),
                                      ('F', 'Fall Semester'), ('S', 'Spring Semester')], validators=[DataRequired()])
